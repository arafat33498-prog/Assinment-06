...
const SingleCard = ({ cards, handleAddToCart }) => {
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm relative flex flex-col justify-between">
            {cards.badge && (
            ...
            <div>
                ...
                <p className="text-slate-500 text-sm leading-relaxed mb-6">{cards.description}</p>
            ...
            <button 
                className={`w-full font-bold py-4 rounded-full transition-all duration-200 ${
                    isSelected ? 'bg-green-500 text-white cursor-default' : 'bg-[#8B2CFF] hover:bg-[#7a25e0] text-white'
                }`} 
    ...
    return (
        <div className="container mx-auto">
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {card.map((item, index) => (
                    <SingleCard key={index} cards={item} handleAddToCart={handleAddToCart} />
                ))}
            </div>
        </div>
    );
